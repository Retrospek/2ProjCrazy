...
def process_words():
    ...
    word_vectors_input = input("Enter word vectors as a 2D list string: ") ####################### INPUTS #######################
    ...
    while strikes < 4 and remaining_attempts > 0 and len(correctGroups) < 4:
        ...
        try:
            ...
            print(f"Response from model: {response}")               ############################## So this is the output #############
            ...
            try:
                ...
                is_correct = input(f"Is this guess correct? (True/False): ").strip().lower() ####################### WETHER THE INPUT WAS CORRECT OR NOT ################
                ...
                if is_correct == 'true':
                    ...
                    isOneAway = input("Is one away? (True/False): ").strip().lower() == 'true' ################# THEM TELLING IF ONE AWAY #############
                    
                    if isOneAway:
                        # Increment strikes for "one away"
                        strikes += 1
                        print(f"Strike added for 'one away' guess. Total Strikes: {strikes}") ############ MARKING STRIKES
                        ...
                        if current_attempt in failed_one_away_attempts:
                            print("This combination has already failed in a one-away attempt") ############# WARNING FOR COMBO NOT WORKING ########
                            continue
                        ...
                        print(f"Wrong guess. Strikes: {strikes}") ###########WARNING FOR WRONG GUESS #########
                
                all_categorizations.append(f"Attempt {len(previousGuesses)}: {guessed_group}")
                
            except Exception as e:
                print(f"Error parsing response: {e}") ######### ERROR HANDLING
                strikes += 1
